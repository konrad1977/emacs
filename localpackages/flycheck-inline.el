...
(cl-defun flycheck-inline--mark-all-symbols (&key input regex property)
  "Highlight all symbols matching REGEX in INPUT with specified PROPERTY."
  (save-match-data
    (setq input (replace-regexp-in-string
                 regex
                 (lambda (match)
                   (let* ((start-position (match-beginning 1))
                          (end-position (match-end 1))
                          (existing-face (text-properties-at start-position input))
                          (new-face (append existing-face (list 'face property))))
                     (add-text-properties start-position end-position new-face match)
                     (replace-match (propertize (match-string 1 match) 'face new-face) nil nil match)))
                 input)))
  input)

(cl-defun mark-all-symbols (&key input regex property)
  "Apply face properties to matched symbols in the given text, preserving the background color."
  (with-temp-buffer
    (insert input)
    (goto-char (point-min))
...
(defun flycheck-inline-remove-client (text)
  "Remove the leading word up to the first colon in the MESSAGE."
